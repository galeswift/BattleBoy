...
protected:
    struct STATE
    {
        ...
        LPDXUTCALLBACKCHAR m_CharFunc;             // char callback
        LPDXUTCALLBACKMOUSE m_MouseFunc;                // mouse callback
        LPDXUTCALLBACKMSGPROC m_WindowMsgFunc;            // window messages callback
        LPDXUTCALLBACKASKSCREENSIZE m_AskScreenSizeFunc;            // screen size callback
        ...
        void* m_CharFuncUserContext;					 // user context for char callback
        ...
        void* m_AskScreenSizeFuncUserContext;            // user context for screen size callback
...
public:
    ...
    GET_SET_ACCESSOR( LPDXUTCALLBACKCHAR, CharFunc );
    GET_SET_ACCESSOR( LPDXUTCALLBACKMOUSE, MouseFunc );
    GET_SET_ACCESSOR( LPDXUTCALLBACKMSGPROC, WindowMsgFunc );
    GET_SET_ACCESSOR( LPDXUTCALLBACKASKSCREENSIZE, AskScreenSizeFunc );
    ...
    GET_SET_ACCESSOR( void*, CharFuncUserContext );
    GET_SET_ACCESSOR( void*, MouseFuncUserContext );
    GET_SET_ACCESSOR( void*, WindowMsgFuncUserContext );
    GET_SET_ACCESSOR( void*, AskScreenSizeFuncUserContext );
...
LRESULT CALLBACK DXUTStaticWndProc( HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam )
{
    ...
    // Consolidate the character messages and pass them to the app's character callback
	if( uMsg == WM_CHAR)
    {
...
HRESULT WINAPI DXUTToggleFullScreen()
{
    ...
    UINT nWidth = ( bIsWindowed ) ? 
		GetDXUTState().GetWindowBackBufferWidthAtModeChange() :
        GetDXUTState().GetFullScreenBackBufferWidthAtModeChange();
    UINT nHeight = ( bIsWindowed ) ? 
		GetDXUTState().GetWindowBackBufferHeightAtModeChange() :
